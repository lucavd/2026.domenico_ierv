...
beta_strategy <- c("A" = 0, "B" = 0, "C" = 0, "D" = 0)   # main effect of strategies
beta_subgroup <- c("ARDS" = 0, "non-ARDS" = 0)           # subgroup penalty
beta_stage1 <- c("A" = 0, "B" = 0)
...
generate_utility <- function(strategy, group,stage1) {
  ...
  return(rnorm(1, mu, 1)) # noise SD=1
  #return(mu)
}
...
simulatore<-function(n_t){
  ...
  for (i in 1:n_simulations) {
    ...
    model <- stan_glmer(
      Outcome ~ 0 + Strategy + (0 + Strategy | Subgroup),
      data = trial_data,
      family = gaussian(),
...
write_csv(df_finale, "risultati_simulazione_alpha_bayes.csv")
